...
if ($config["url_rewrite"]) {
    ...
    $url_path["masternodes"] = $config["root_path"] . $config["explorer_path"] . 'masternodes/';
    $url_path["richlist"] = $config["root_path"] . $config["explorer_path"] . 'richlist/';
} else {
    ...
    $url_path["masternodes"] = $config["root_path"] . $config["explorer_path"] . '?action=masternodes&v=';
    ...
    case "height":
        ...
        if (count($output['blocks']) > 0) {
            foreach ($output['blocks'] as $value) {
                $output['block_list_tbody'] .= "<tr><td><a class=\"text-info\" href=\"" . $url_path["block"] . $value["height"] . "\">" . $value["height"] . "</a></td><td><a class=\"text-info\" href=\"" . $url_path["blockhash"] . $value["hash"] . "\">" . $value["hash"] . "</a></td><td>" . $value["date"] . "</td><td>" . $value["tx_count"] . "</td><td>" . $value["value_out"] . "</td><td>" . $value["size"] . "</td></tr>";
            }
        }
    ...
    case "masternodes":
        ...
        if (count($output['listmaster']) > 0) {
            ...
            if ($value["status"] == "ENABLED"){
                ...
                $output['masternode_list_tbody'] .= "<tr><td>" . $value["rank"] . "</td><td>" . $value["network"] . "</td><td>" . $value["address"] . "</td><td>" . $value["ipport"] . "</td><td>" . $value["version"] . "</td><td>" . $value["lastpaid"] . "</td><td class=". $class .">" . $value["status"] . "</td></tr>";
            }
        }
    exit(get_html("masternodes", $output));
        break;
